...
class ApplyFlintSparkCoveringIndexSuite extends FlintSuite with Matchers {
  ...
  /** Mock OpenSearchCluster to avoid looking for real OpenSearch cluster */
  private val openSearchCluster = mockStatic(classOf[OpenSearchCluster])
  ...
  override protected def beforeAll(): Unit = {
    ...
    // Mock static
    openSearchCluster
      .when(() => OpenSearchCluster.apply(any(classOf[String]), any(classOf[FlintOptions])))
      .thenCallRealMethod()
      ...
      indexes.foreach { index =>
        {
          openSearchCluster
            .when(() =>
              OpenSearchCluster
                .getAllOpenSearchTableMetadata(any(classOf[FlintOptions]), mockitoEq(index.name)))
            .thenReturn(
              Seq(
                MetaData(
                  index.name,
                  generateSchemaJSON(index.indexedColumns),
                  index.metadata.indexSettings.getOrElse(""))).asJava)
        }
      }
      rule.apply(plan)
    }
